...
def showimg_WS(resultDir,start, end, obsdata, simdata):

    imgdir = resultDir+"\\imgs"
    wsdir  = resultDir+"\\imgs\\WS"
    ...
    for ston in stons:
        ...
        plt.ylabel('Wind Speed', fontsize=20)  # 給左邊的Y軸說明
        plt.title( ston+"\n"+start.strftime('%Y-%m-%d')+"至"+end.strftime('%Y-%m-%d')+" \n 日平均風速", fontsize=20)
        plt.legend(loc='best', fontsize=17)
        # plt.xticks(rotation=70)
        plt.savefig(wsdir+'\\'+ start.strftime('%Y-%m-%d')+"_"+end.strftime('%Y-%m-%d')+"average"+ston+".png", dpi=500, format="png")
