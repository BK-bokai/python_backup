...
def MergeTxt_obs(start,end,workdir):
    ...
    while (start <= end):
        ...
        obs_out = pd.read_excel("D:\\bokai\\python\\python-code\\氣象性能評估工具V2\\data\\obs\\T2\\"+filetimes[0]+"_T2_obs.xlsx")


    for i in range(1,len(filetimes)):
        obs_in = pd.read_excel("D:\\bokai\\python\\python-code\\氣象性能評估工具V2\\data\\obs\\T2\\"+filetimes[i]+"_T2_obs.xlsx")
