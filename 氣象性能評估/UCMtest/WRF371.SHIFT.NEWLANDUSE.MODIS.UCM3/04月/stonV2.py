...
obs = pd.read_excel("201604obs.xlsx")
sim = pd.read_excel("201604sim.xlsx")
...
for ston in stons:
    ...
    plt.savefig("D:\\bokai\\python\\python-code\\氣象性能評估\\UCMtest\\WRF371.SHIFT.NEWLANDUSE.MODIS.UCM3\\4月\\4月img\\2016-" +
            x2[0][0:2]+ston+".png", dpi=500, format="png")



with open("201604obs_avg.csv", 'w', newline='') as csvFile:
...
with open("201604sim_avg.csv", 'w', newline='') as csvFile:
    writer = csv.writer(csvFile)
    writer.writerow(['UTC','台北', '板橋', '新竹', '宜蘭', '花蓮', '梧棲', '台中', '嘉義', '永康', '台南', '台東', '高雄', '恆春'])
    for i in range(0,24):
