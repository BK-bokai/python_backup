...
sim = pd.read_excel("201606sim.xlsx")
...
for ston in stons:
    ...
    stons_obs_avg[ston] =ston_obs_avg[0:24]/30
    
    ston_sim_avg = np.zeros(24)
    for i in range(0, 720, 24):
        ston_sim_avg[0:24] = sim[ston][i:i+24]+ston_sim_avg[0:24]
    stons_sim_avg[ston] =ston_sim_avg[0:24]/30
    ...
    plt.savefig("D:\\bokai\\python\\python-code\\氣象性能評估\\UCMtest\\WRF371.SHIFT.NEWLANDUSE.MODIS.UCM3\\7月\\6月img\\2016-" +
            x2[0][0:2]+ston+".png", dpi=500, format="png")



with open("201606obs_avg.csv", 'w', newline='') as csvFile:
