...
obs = pd.read_excel("201607obs.xlsx")
...
for ston in stons:
    ...
    stons_sim_avg[ston] =ston_sim_avg[0:24]/31
    ...
    # D:\bokai\python\python-code\氣象性能評估\UCMtest\to_ycchen\678\201608sim_avg.csv
    plt.savefig("D:\\bokai\\python\\python-code\\氣象性能評估\\UCMtest\\to_ycchen\\678\\7月img\\2016-" +
            x2[0][0:2]+ston+".png", dpi=500, format="png")



with open("201607obs_avg.csv", 'w', newline='') as csvFile:
...
with open("201607sim_avg.csv", 'w', newline='') as csvFile:
    writer = csv.writer(csvFile)
    writer.writerow(['UTC','台北', '板橋', '新竹', '宜蘭', '花蓮', '梧棲', '台中', '嘉義', '永康', '台南', '台東', '高雄', '恆春'])
    for i in range(0,24):
