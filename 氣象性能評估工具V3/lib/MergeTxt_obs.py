...
def MergeTxt_obs(start, end, workdir, var):
    ...
    while (start <= end):
        ...
        start += datetime.timedelta(days=1)
        obs_out = pd.read_excel("D:\\bokai\\python\\python-code\\氣象性能評估工具V3\\data\\obs\\"+var+"\\"+filetimes[0]+"_"+var+"_obs.xlsx")


    for i in range(1,len(filetimes)):
        obs_in = pd.read_excel("D:\\bokai\\python\\python-code\\氣象性能評估工具V3\\data\\obs\\"+var+"\\"+filetimes[i]+"_"+var+"_obs.xlsx")
