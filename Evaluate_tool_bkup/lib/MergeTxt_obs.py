...
def MergeTxt_obs(start, end, rootpath, workdir, var):
    ...
    while (start <= end):
        ...
        start += datetime.timedelta(days=1)
        obs_out = pd.read_excel(rootpath+"\\data\\obs\\"+var+"\\"+filetimes[0]+"_"+var+"_obs.xlsx")


    for i in range(1,len(filetimes)):
        obs_in = pd.read_excel(rootpath+"\\data\\obs\\"+var+"\\"+filetimes[i]+"_"+var+"_obs.xlsx")
